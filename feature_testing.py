...
def main():
    ...
    imp_df = permutation_importance(X_train_f, X_val_f, X_test_f, y_test, contamination=contamination, random_state=42)
    imp_df.to_csv(os.path.join(OUT_DIR, "permutation_importance_full.csv"), index=False)
    print(f"Saved: {os.path.join(OUT_DIR, 'permutation_importance_full.csv')}")

    for k in (5, 10, 15):
        imp_df["feature"].iloc[:k].to_frame().to_csv(os.path.join(OUT_DIR, f"best_features_k{k}.csv"), index=False)
        print(f"Saved top-{k} features: {os.path.join(OUT_DIR, f'best_features_k{k}.csv')}")

    print("Retraining and evaluating baseline vs reduced feature sets...")
    n = len(X_train_f.columns)
    k_candidates = sorted({5, 10, 15, max(1, n // 4), max(1, n // 2), max(1, (3 * n) // 4)})
    results_df = retrain_and_evaluate(X_train_f, X_val_f, X_test_f, y_train, y_test, imp_df, k_candidates)
    results_df.to_csv(os.path.join(OUT_DIR, "retrain_results_full.csv"), index=False)
    print(f"Saved: {os.path.join(OUT_DIR, 'retrain_results_full.csv')}")
    ...
    summary_path = os.path.join(OUT_DIR, "retrain_results_full.txt")
